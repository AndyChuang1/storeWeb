...
module.exports = {
  connectDB() {
    db.serialize(() => {
      db.run(
        'CREATE TABLE IF NOT EXISTS product(name TEXT PRIMARY KEY,unit TEXT,types TEXT,price INTEGER,detail TEXT,path TEXT,sales INTEGER)'
      );
      ...
      db.run(`CREATE TABLE IF NOT EXISTS deliver
            (
              type TEXT PRIMARY KEY, 
              main  INTEGER NOT NULL,
              offshore INTEGER NOT NULL,
    ...
    db.serialize(() => {
      db.run(sqlStatement, [type, main, offshore, extra], function(err, rows) {
        if (err) {
          ...
          cb({ success: true });
          console.log('insertDeliverFee : ' + type);
        }
      });
    });
  },
